package com.example.demo.poi.watermark;
...
public class WordWaterMarker{
    ...
    private int widthPerWord = 8; // 一个字平均长度，单位pt，用于：计算文本占用的长度（文本总个数*单字长度）, 默认为10
    private String styleTop = "0"; // 与顶部的间距
    private String styleRotation = String.valueOf(360-45); // 文本旋转角度
    private String totalWidth = "700pt";

    public WordWaterMarker(String customText) {
        customText = customText + repeatString(" ", 30); // 水印文字之间使用8个空格分隔
        this.customText = repeatString(customText, 4); // 一行水印重复水印文字次数, 默认为10
    }

    public static void main(String[] args) throws FileNotFoundException{
        WordWaterMarker m = new WordWaterMarker("兰众科技");
        FileInputStream fileInputStream = new FileInputStream("F:/TEMP/审批单模板.docx");
        FileOutputStream fileOutputStream = new FileOutputStream("F:/TEMP/审批单模板1.docx");
        m.makeSlopeWaterMark(fileInputStream, fileOutputStream);
    }
    ...
    private String getShapeStyle() {
        ...
        sb.append(";width: ").append(customText.length() * widthPerWord); // 计算文本占用的长度（文本总个数*单字长度）
